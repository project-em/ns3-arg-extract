...
class NlpFeatures:
	...
	def __init__(self, word2vec_filepath, lexicon_filepath):
		self.word2vec = Word2Vec.load_word2vec_format(word2vec_filepath, binary=True)
	...
	def sentiment_scores(self, sentences):
		sent_analyzer = SentimentIntensityAnalyzer(lexicon_file=self.lexicon_filepath)
	...
	def featurize(self, topics, sentences):
		...
		print "creating noun similarity feature..."
		featurized[:,0] = self.nouns_sim(sentences, tagged_sentences, topics, tagged_topics)
		print "creating sentiment scores feature..."
